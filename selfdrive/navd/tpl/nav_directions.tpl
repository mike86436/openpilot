  ...
  <div style="padding: 5px; font-weight: bold;" align="center">
  <a href="?reset=1"><img style="width: 50px; height: 50px; background-color: black;" src="logo.png"></a>
  </div>
        ...
        async function fetchAndDisplayData() {
            try {
                ...
                for (const step of steps) {
                    const bannerInstructions = step.bannerInstructions;

                    for (const banner of bannerInstructions) {
                        const primary = banner.primary;

                        // Access the values directly under primary
                        const typeValue = primary.type;
                        const modifierValue = primary.modifier;
                        const textValue = primary.text;
                        const distanceMeters = banner.distanceAlongGeometry;
                        const distanceMiles = distanceMeters * 0.000621371;

                        // Display the values on the webpage
                        jsonOutputDiv.innerHTML += `
                            <p>In ${distanceMiles.toFixed(1)} miles ${typeValue} ${modifierValue} : ${textValue}</p>
                            <hr>
                        `;
                    }
                }
